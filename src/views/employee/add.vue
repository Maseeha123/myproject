<template>
  ...
  <v-card>
    <v-card-text>
     <formFields :formFields="fields" @save="saveUpdateEmployeeDetails" :record='records' @back="backToEmployee" :cancel="cancelButton" :buttons="true" :isButtonShow="isButtonShow"></formFields>
    </v-card-text>
  </v-card>
   </div>
</template>
    ...
    saveUpdateEmployeeDetails (employee) {
      if (employee) {
        ...
        if (employee.id === this.$route.params.id) {
          this.saveUpdate(this.url, employee).then(() => {
            this.$root.$emit('snackBar', { text: 'Employee Added', color: 'green darken-1', snackBar: true })
            this.backToEmployee()
          })
        } else {
          this.saveUpdate(this.url, employee, this.$route.params.id).then(() => {
