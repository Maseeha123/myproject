<template>
  <v-app>
   <div  v-if="form">
     <v-btn color="primary" class="float-left ml-2" @click="backToTask()" dark> <v-icon dark left> mdi-arrow-left</v-icon> Back</v-btn>
     </div>
        <div v-if="form" class="mt-4">
        <v-card :disabled="$store.state.userDetails.admin === true ? false : true">
          <v-card-text>
            <formFields :formFields="fields" @save="saveUpdatTask" @back="backToTask" :record="records" :buttons="true" :isButtonShow="isButtonShow" :cancel="true"></formFields>
          </v-card-text>
        </v-card>
        </div>
        <div v-if="showDetails">
      <v-btn color="primary" class="float-left mr-4 mt-2" :disabled="disableButton" @click="showHideDialog = true" >Log Time</v-btn>
        <v-row justify="end">
        <v-btn color="primary" v-if="table" @click="table = false;form = true;editTask()" class=" float-right mt-6 mr-6">Back</v-btn>
      </v-row>
      <v-row justify="center">
        <v-dialog v-model="showHideDialog" v-if="showHideDialog" persistent max-width="700px">
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <formFields :formFields="logTimefields" @save="savelogHours" :buttons="true" :record="records" @hideDialog="hideDialog" :isButtonShow="isButtonShow" ref="clear"></formFields>
              </v-container>
            </v-card-text>
          </v-card>
        </v-dialog>
      ...
      <v-row v-if="table">
        <v-col cols="12">
          <renderTable :tableHeader="hoursTableHeader" :loading="true" :tableBody="hoursTableBody" :addButton="false" :showTable="true" isEditable:isEditable />
        </v-col>
      </v-row>
    </div>
  </v-app>
